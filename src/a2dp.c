...
#define OPTIMAL_FRAMES_MIN 50
#define OPTIMAL_FRAMES_MAX 90
...
static btstack_timer_source_t _health_timer;
...
uint8_t _decoded_audio_storage[(256+32) * BYTES_PER_FRAME] = {0};
...
static void handle_pcm_data(int16_t * data, int num_audio_frames, int num_channels, int sample_rate, void * context) {
    ...
    int16_t  output_buffer[(256+32) * NUM_CHANNELS]; // increased buffer
...
// Forward declarations for the watchdog
...
// Auto-recovery: soft reset media if buffer drifts badly during playback
static void buffer_health_check(btstack_timer_source_t *ts) {
    if (_audio_stream_started && _sbc_frame_size > 0) {
        int sbc_frames = btstack_ring_buffer_bytes_available(&_sbc_frame_ring_buffer) / _sbc_frame_size;

        // if buffer is critically empty or overflowing — soft reset media
        if (sbc_frames < 10 || sbc_frames > OPTIMAL_FRAMES_MAX + ADDITIONAL_FRAMES - 10) {
            printf("HEALTH: buffer at %d frames, soft reset\n", sbc_frames);
...
static void media_processing_start(void) {
    ...
    // start buffer health watchdog
    btstack_run_loop_remove_timer(&_health_timer);
    btstack_run_loop_set_timer_handler(&_health_timer, buffer_health_check);
    btstack_run_loop_set_timer(&_health_timer, 2000);  // first check after 2s
    btstack_run_loop_add_timer(&_health_timer);
}
...
static void media_processing_pause(void) {
    ...
    _audio_stream_started = false;
    btstack_run_loop_remove_timer(&_health_timer);  // stop watchdog
...
static void event_handler(uint8_t event, uint8_t *packet) {
    ...
    switch (event){
        ...
        case A2DP_SUBEVENT_STREAM_RELEASED:
            ...
            media_processing_close();
            led_connected(false);  // return to pairing mode when the stream disconnects
            gpio_put(CONN_PIN, 0);
            break;
        ...
        case A2DP_SUBEVENT_SIGNALING_CONNECTION_RELEASED:
            ...
            _stream_state = STREAM_STATE_CLOSED;
            media_processing_close();
            led_connected(false);  // return to pairing mode when the phone disconnects
            gpio_put(CONN_PIN, 0);
            break;
...
static void media_handler(uint8_t seid, uint8_t *packet, uint16_t size) {
    ...
    uint32_t compensation   = 0x00120;  // Auto-configured
